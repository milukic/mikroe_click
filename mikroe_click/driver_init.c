...
struct usart_async_descriptor       clickUART;
...
static uint8_t clickUART_buffer[DEBUGUART_BUFFER_SIZE];
 ...
 */
void clickUART_init(void)
{
	clickUART_CLOCK_init();
	usart_async_init(&clickUART, SERCOM3, clickUART_buffer, DEBUGUART_BUFFER_SIZE, (void *)NULL);
	clickUART_PORT_init();
}
...
void system_init(void)
{
	...
	// Set pin direction to output
	gpio_set_pin_direction(CLICK_EN, GPIO_DIRECTION_OUT);

	//gpio_set_pin_function(CLICK_EN, GPIO_PIN_FUNCTION_OFF);
