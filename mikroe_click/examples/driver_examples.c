...
/**
 * Example of using clickUART to write "Hello World" using the IO abstraction.
 *
...
static uint8_t example_clickUART[12] = "Hello World!";
...
void clickUART_example(void)
{
	struct io_descriptor *io;

	usart_async_register_callback(&clickUART, USART_ASYNC_TXC_CB, tx_cb_clickUART);
	/*usart_async_register_callback(&clickUART, USART_ASYNC_RXC_CB, rx_cb);
	usart_async_register_callback(&clickUART, USART_ASYNC_ERROR_CB, err_cb);*/
	usart_async_get_io_descriptor(&clickUART, &io);
	usart_async_enable(&clickUART);

	io_write(io, example_clickUART, 12);
}
